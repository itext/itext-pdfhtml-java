...
pipeline {
    ...
    stages {
            ...
            steps {
                withMaven(jdk: "${JDK_VERSION}", maven: 'M3') {
                    withSonarQubeEnv('Sonar') {
                        sh 'mvn --activate-profiles test -DgsExec="${gsExec}" -DcompareExec="${compareExec}" -Dmaven.test.skip=false -Dmaven.test.failure.ignore=false -Dmaven.javadoc.skip=true -Dspotbugs.skip=true org.jacoco:jacoco-maven-plugin:prepare-agent verify org.jacoco:jacoco-maven-plugin:report sonar:sonar ' + sonarBranchName + ' ' + sonarBranchTarget
                    }
                }
            }
        }
        stage('Static Code Analysis') {
            ...
            steps {
                withMaven(jdk: "${JDK_VERSION}", maven: 'M3') {
                    sh 'mvn --activate-profiles qa verify -Dpmd.analysisCache=true -Dfindbugs.skip=true'
                }
            }
        }
        stage("Quality Gate") {
